...
pacman::p_load(corrplot, randomForest, rmarkdown, foreach, plyr, tidyverse, magrittr, data.table, dplyr, tibble, caret)
```
...
lmp.fit=lm(int_rate~., data=subset(dataset, select=c(-id_2, -emp_title, -hardship_type, -issue_d, -desc, -title, -zip_code, -earliest_cr_line, -total_rec_late_fee, -last_pymnt_d, -next_pymnt_d, -last_credit_pull_d, -policy_code, -sec_app_earliest_cr_line, -debt_settlement_flag_date, -settlement_date, grade)), na.action = na.exclude)
...
int_rate_RF <- randomForest(int_rate ~ sub_grade + loan_status + total_pymnt + installment + initial_list_status + total_pymnt_inv + verification_status + loan_amnt + term + out_prncp_inv + last_pymnt_amnt + num_tl_op_past_12m  + num_bc_tl + out_prncp + pub_rec + total_bc_limit + open_acc + num_sats + purpose + total_rec_prncp, data=dataset, subset = small_trainRows, importance=TRUE, do.trace = TRUE, na.action = na.exclude)
int_rate_RF
summary(int_rate_RF)
plot(int_rate_RF)
