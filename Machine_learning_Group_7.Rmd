...
```{r Initilize the environment, warning=TRUE}
  ...
  pacman::p_load(import, monomvn, party, dummies, ranger, data.table, rmarkdown, tidyverse, caret, pls, corrplot, randomForest, foreach, plyr, tidyverse, magrittr, dplyr, tibble, doMC, pROC, class,MLmetrics, tree, car, ridge, lmridge, xgboost)
```
...
if (file.exists("fit.egb.reg.rds")) {
   fit.lm.reg <- readRDS("fit.egb.reg.rds")
} else {
...
saveRDS(fit.egb.reg, "fit.egb.reg.rds")
...
# MAPE ExtremGradiantBoosting TEST
MAPE(predict(fit.egb.reg, test.data.reg), test.data.reg$int_rate)
...
MAPE(predict(fit.egb.reg, train.data.reg), train.data.reg$int_rate)
...
Furthermore it would be intersting to spend more time on the performance tuning of each model, by using different parmaters for the fitting. 
