...
dataset_ctrl <- trainControl(method = "repeatedcv", number = 10, repeats = 3)
model_caret <- train(int_rate~., data = test.data.reg, trControl = dataset_ctrl, method = "lm", na.action = na.pass)  
model_caret
...
```{r Bayesian Generalized Linear Model analysis regression}

# Train glmnet model
glmnet_reg <- train(defaulted ~ ., data = train.data.reg,
 method = "bayesglm",
