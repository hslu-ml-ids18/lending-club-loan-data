...
What is the project about? What is our action plan?
...
null_model <- glm(int_rate ~ 1, data = dataset_reg[small_trainRows,], family = "gaussian")

# Specify the partial model using a subset of the potential predictors
partial_model <- glm(int_rate ~ ., data= dataset_reg[small_trainRows,], family = "gaussian")
...
```{r stepwise model Both direction elimination , results = FALSE, cache=TRUE, eval=FALSE}
# should we use a gaussian model here?
null_model <- glm(int_rate ~ 1, data = dataset_reg[small_trainRows,], family = "gaussian")
...
train.data <- dataset_cla[trainIndex,]
test.data  <- dataset_cla[-trainIndex,]

```
## Data Exploration
```{rData Exploration Correlation}
