...
pacman::p_load(corrplot, randomForest, rmarkdown, foreach, plyr, tidyverse, magrittr, data.table, dplyr, tibble, caret, foreach, doMC, caret, MLmetrics)
#Register 4 Cores
registerDoMC(4)
...
small_trainRows = sample(1:nrow(dataset),0.01*nrow(dataset))
small_testRows = sample(nrow(dataset) - small_trainRows,0.01*nrow(dataset))
```
...
```{r stepwise model, results = FALSE}
# should we use a gaussian model here?
null_model <- glm(int_rate ~ 1, data = subset(dataset[small_trainRows,], select=c(-id_2, -policy_code)), family = "gaussian")

# Specify the partial model using a subset of the potential predictors
partial_model <- glm(int_rate ~ ., data=subset(dataset[small_trainRows,], select=c(-id_2, -policy_code)), family = "gaussian")
...
model2 <- train(int_rate~., data = dataset[small_trainRows,], method = "ranger")
plot(model2)
```
