...
In order to avoid to have to work with the whole original dataset_reg from https://www.kaggle.com/wendykan/lending-club-loan-data
dataset_reg_7.Rds has been created as follows.
...
dataset_reg_7 <- loan_df[which(loan_df$id_2%%8+1== 7),]
saveRDS(dataset_reg_7, file = "dataset_reg_7.Rds")

dataset_reg_7 <- readRDS(file = "dataset_reg_7.Rds")
```


  dataset_reg <- readRDS(file = "dataset_reg_7.Rds")
...
sort dataset_reg by colunm names
  dataset_reg = dataset_reg[ , order(names(dataset_reg))]
  
  #removing policy_code because always equals 1
  dataset_reg = subset(dataset_reg, select = -c(policy_code) )
  
  
  # Removing columns that have > 0.05 NAs
  dataset_reg <- dataset_reg[, -which(colMeans(is.na(dataset_reg)) > 0.05)]
  
  # Omiting columns that only have NAs, ie. id and member_id.
  dataset_reg <- dataset_reg[,colSums(is.na(dataset_reg))<nrow(dataset_reg)]
  ...
  dataset_reg <- subset(dataset_reg, select = -c(id_2, desc, emp_title, issue_d, title, zip_code, last_pymnt_d, next_pymnt_d, last_credit_pull_d, hardship_end_date, hardship_start_date, payment_plan_start_date, debt_settlement_flag_date, settlement_date, addr_state, grade) )

  # sub_grade has more than 32 levels which is a hard limit for random forest.
  # We'll dummy code it to circument this
  levels_sub_grade <- levels(dataset_reg$sub_grade)
  dataset_reg$sub_grade<- as.numeric(mapvalues(dataset_reg$sub_grade, levels_sub_grade, seq(from = 1, to = 35, by = 1)))
  levels(dataset_reg$sub_grade)
  
  # sorting emp_lenght, and dummy coding.
  levels_emp_length <- levels(dataset_reg$emp_length)
  dataset_reg$emp_length <- ordered(dataset_reg$emp_length, levels = c("n/a", "< 1 year", "1 year", "2 years", "3 years","4 years", "5 years","6 years", "7 years", "8 years", "9 years", "10+ years"))
  levels(dataset_reg$emp_length)
  dataset_reg$emp_length <- as.numeric(mapvalues(dataset_reg$emp_length, levels_emp_length, c(-1, seq(from = 0, to = 10, by = 1))))
  levels(dataset_reg$emp_length)
  
  # grouping earliest_cr_line by year
  dataset_reg$earliest_cr_line <- as.integer(substring(dataset_reg$earliest_cr_line, 5))
  
  # grouping earliest_cr_line by year
  dataset_reg$sec_app_earliest_cr_line <- as.integer(substring(as.character(dataset_reg$sec_app_earliest_cr_line), 5))
  dataset_reg$sec_app_earliest_cr_line[is.na(dataset_reg$sec_app_earliest_cr_line)] <- -1
  typeof(dataset_reg$sec_app_earliest_cr_line)
  levels(dataset_reg$sec_app_earliest_cr_line)
  
  levels(dataset_reg$total_rec_late_fee)
  
  na_count <-sapply(dataset_reg, function(y) sum(length(which(is.na(y)))))
  
  # changing the dataset_reg as a tbl object.
  dataset_reg <- as.tbl(dataset_reg)
  # change NAs to 0 in integer columns
  dataset_reg <- mutate_if(dataset_reg, is.integer, ~replace(., is.na(.), -1))
  # change NAs to 0 in doubles columns
  
  dataset_reg <- mutate_if(dataset_reg, is.numeric, ~replace(., is.na(.), -1))
  # change NAs to 0 in strings columns
  
  dataset_reg <- mutate_if(dataset_reg, is.character, ~replace(., is.na(.), "NA"))
  
  # change NAs to 0 in strings columns
  
  # dataset_reg <- mutate_if(dataset_reg, is.factor, ~replace(., is.na(.), "NA"))
  
  # change columns to character¨
  # dataset_reg$hardship_type <- as.character(dataset_reg$hardship_type)
  # dataset_reg$hardship_reason <- as.character(dataset_reg$hardship_reason)
  # dataset_reg$hardship_status <- as.character(dataset_reg$hardship_status)
  # dataset_reg$hardship_loan_status <- as.character(dataset_reg$hardship_loan_status)
  # dataset_reg$settlement_status <- as.character(dataset_reg$settlement_status)
  # dataset_reg$verification_status_joint <- as.character(dataset_reg$verification_status_joint)
  # 
  ...
  sapply(dataset_reg, function(col) length(unique(col)))
...
## Preparatory tasks:
### Create a copy of your dataset_reg, eliminating the entries that have an “na” in the interest rate variable int_rate. (Interest rate is used as output variable).
## Initialization

```{r preprocessing, echo=TRUE}
# Creating a separate dataset_reg for regression
dataset_reg_reg <- dataset_reg
