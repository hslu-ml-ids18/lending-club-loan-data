...
```{r Initilize the environment, warning=TRUE}
  ...
  pacman::p_load(import, monomvn, party, dummies, ranger, data.table, rmarkdown, tidyverse, caret, pls, corrplot, randomForest, foreach, plyr, tidyverse, magrittr, dplyr, tibble, doMC, pROC, class,MLmetrics, tree, car, ridge, lmridge)
```
...
#dataset_reg <- subset(dataset_reg, select = c(int_rate, bc_util, delinq_2yrs, installment, initial_list_status, loan_status, revol_util, sub_grade, total_pymnt_inv, out_prncp, open_acc, mo_sin_rcnt_rev_tl_op, total_rec_int, pct_tl_nvr_dlq, num_il_tl))
...
```{r Generalized Linear Model with Stepwise Feature Selection, cache=TRUE}
...
```
```{r Bayesian Linear Ridge Regression, cache=TRUE}

#fit.bridge.reg <- train(int_rate~.,
#                    data = test.data.reg,
#                    trControl = control.reg,
#                    method = "bridge",
#                    na.action = na.pass)
#fit.bridge.reg
#fit.bridge.reg$finalModel
#plot(fit.bridge.reg$finalModel)
```

```{r Random Forest}
## Read Model from file or compute it 
if (file.exists("fit.RF.reg.rds")) {
   fit.bridge.reg <- readRDS("fit.RF.reg.rds")
} else {
...
# MAPE glmStepAIC TEST
mape_glmStepAIC_test <- MAPE(predict(fit.glmStepAIC.reg, test.data.reg), test.data.reg$int_rate)

# MAPE bridge TEST
mape_bridge_test <- MAPE(predict(fit.bridge.reg, test.data.reg), test.data.reg$int_rate)

# MAPE Random Forest TEST
mape_rf_test <- MAPE(predict(fit.RF.reg, test.data.reg), test.data.reg$int_rate)

test_mapes <- data.frame("LM" = mape_lm_test, "glmStep"= mape_glmStepAIC_test, "Brdige"= mape_bridge_test, "Random Forest" = mape_rf_test)
```

```{r Calculating TRAIN MAPE out of the models}
# MAPE LM train
MAPE(predict(fit.lm.reg, train.data.reg), train.data.reg$int_rate)

# MAPE glmStepAIC train
MAPE(predict(fit.glmStepAIC.reg, train.data.reg), train.data.reg$int_rate)

# MAPE Randdom Forest train
MAPE(predict(fit.RF.reg, train.data.reg), train.data.reg$int_rate)

train_mapes <- data.frame("LM" = mape_lm_train, "glmStep"= mape_glmStepAIC_train, "Random Forest" = mape_rf_train)
```
...
print(mapes)
# # Plotting the Test and Train MAPEs
# plot(mapes)
# 
# {ggplot(mapes, aes(x = dataset_reg)) + 
#   geom_point()}
...
#plot(varImp(fit.lda.cla), 5, main = "PCA-DA")
...
## logistic regression
fit.lreg.cla <-train(defaulted ~ recoveries + collection_recovery_fee + total_rec_prncp + last_pymnt_amnt + total_pymnt, 
                data = train.data.cla, 
                method = "glm",
                trControl = control.cla,
                family=binomial(),
...
## decision tree 
fit.dtree.cla <-train(defaulted ~ recoveries + collection_recovery_fee + total_rec_prncp + last_pymnt_amnt + total_pymnt, 
               data=train.data.cla,
               method="ctree",
               trControl = control.cla,
               preProc = c("zv","center","scale"))
...
## random forest 
fit.rf.cla <-train(defaulted ~ recoveries + collection_recovery_fee + total_rec_prncp + last_pymnt_amnt + total_pymnt, 
               data=train.data.cla,
               method="rf",
               trControl = control.cla,
               preProc = c("zv","center","scale"))
...
```{r ROC Plot}
par(pty = "s")
roc(test.data.cla$defaulted, as.numeric(predict(fit.dtree.cla, test.data.cla)), plot=TRUE, legacy.axes=TRUE, percent=TRUE, xlab="False Positive Percentage", ylab="True Postive Percentage", col="#377eb8", lwd=4, print.auc = TRUE, print.auc.y=80, print.auc.x=30)
...
Repacling missing data:
In our case missing data fro int_rate was simply removed, other variables that did have missing data we treated as follow:
- for numericals values we dummy coded them with a '-1' in the approche.
- for factors we replaced missing values with 'unkown'

In the approache we're comparing the int_rate used a similar approache named "Missing Not At Random".
...
In the approache we are comparing outliers were removed. 
 
Sampling:

We first started with a very small subset (1000 rows) to get a hold if the models we were running were doing so correctly. In our final approache we are using 20000 rows as using any more is too computationaly intensive for the hardware we have.

In the approache we are comparing too the whole dataset is of 29092 row. 20000 are used for training and 9092 are used from testing.

Parameter selection 
To find out which parameters were usefull we used a logistic regression and a stepwise regression.

This helped us to select a handfull of predictors that were deemed relevant for our models based upon the p-value.
We can see that this is the same approache that was taken by Matthew Ludwig.
...
Testing:
 ...
 The approache we are comparing to used one model which is as follows:
 
 "Final_Model <- glm(loan_status ~ loan_amnt + grade + annual_inc + int_bin + emp_bin, family = "binomial", data = training)"
# Summary 

We first generated a subset of the orignial lending-club-loan data and saved that file. 

With the new subset we started with exploratory data analysis, based on correlation matrix and, boxplots and plots we could identify importante variables and exclude correlating once. Based on the forgoing data exploration and checking the DataDescription we limited the amount of feautre. In addition we used P For case of simplicity and availalbe computation power we further had to limit the feature. W


There was a imbalance of distribution between good and defaulted loans, but so the analysis show that its helpful with machine learning techniques to predict the interast rate and proability that a loan is defaulted.

## Room for improvement

In future work, natural language processing (NLP) could be used to etraxt informatcion from Loan description or the job title. 
